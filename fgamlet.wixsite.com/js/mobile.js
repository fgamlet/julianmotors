(function() {
  ...
  function fixServiceCards() {
    ...
    // Stack repeater items vertically
    ...
    // Desktop layout maps each repeater item:
    //   Title (ki62mau6): col 1/3, row 1
    //   Icon  (ki63mfpy): col 3,   row 1/6
    //   219zł (ki62maus): col 1,   row 2  |  до20000 (l5pxlp1b): col 2, row 2
    //   439zł (l5pxggj0): col 1,   row 3  |  от20000 (l5pxlp3c): col 2, row 3
    //   649zł (l5pxgyhq): col 1,   row 4  |  от40000 (l5pxlp5a): col 2, row 4
    //  1399zł (m09gpb9x): col 1,   row 5  |  (no desc?)
    ...
    document.querySelectorAll('[id^="comp-ki62mart__item"]').forEach(function(item) {
      ...
      // Fix Wix parent wrappers that add 32px margins (causing card to be narrower than screen)
      ...
      // Title — spans first 2 cols, row 1
      ...
      // Icon — col 3, row 1, aligned to top-right via grid
      ...
      // Helper: apply price and desc styles
      ...
      // Remove card's inner padding-bottom & margin-bottom to reduce gap
      var card = item;
  ...
  function fixSpinningWheel() {
    ...
    // Force the strip and all its children to full width
    ...
    // The columns flex container inside the strip
    if (strip) {
    ...
    // Fix huge min-height and grid-template-rows on inner grid containers (desktop values causing empty gap)
    ...
    // The repeater with service cards inside the strip
    ...
    // Parent section comp-lddi5052 grid — clear its calc margin for strip
  ...
  function fixHeadings() {
    // "Что мы проверяем" heading — inline style="font-size:42px" set directly in HTML, override via JS
    ...
    // "УСЛУГИ" visible heading (comp-mjfw2ks6 is the actual visible richText, comp-ifmfwm06 is hidden)
  ...
  function fixCheckSection() {
    ...
    // Fix outer container offset (desktop has calc((100%-980px)*0.5) + left:78px which breaks mobile)
    var ctr = document.getElementById('comp-ifmdjb8q');
    if (ctr) { s(ctr,'position','relative'); s(ctr,'left','0'); s(ctr,'margin','0'); s(ctr,'width','100%'); }

    // 2-column grid: [50px icons] [1fr labels]
    ...
    // Heading — spans both columns, row 1
    ...
    iconIds.forEach(function(id, i) {
      ...
      // fix all internal Wix wrappers so the icon fills the 70px box
